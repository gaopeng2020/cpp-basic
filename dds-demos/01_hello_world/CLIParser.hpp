...
public:
    ...
    struct subscriber_config : publisher_config {
    ...
    static hello_world_config parse_cli_options(int argc, char* argv[]) {
        ...
        for (int i = 2; i < argc; ++i) {
            ...
            if (arg == "-h" || arg == "--help") {
                ...
                if (i + 1 < argc) {
                    try {
                        if (int input = std::stoi(argv[++i]); input < std::numeric_limits<std::uint16_t>::min() ||
                ...
                if (config.entity == EntityKind::SUBSCRIBER) {
    ...
    /**
     * @brief Parse the signal number into the signal name 将信号编号转换为可读字符串
     *

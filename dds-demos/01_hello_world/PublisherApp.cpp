...
PublisherApp::PublisherApp(const CLIParser::publisher_config& config, const std::string& topic_name) :
    ...
    matched_(0), samples_(config.samples), expected_matches_(config.matched), stop_(false) {
    ...
    // 验证传输配置
    std::cout << "Transport configuration:" << std::endl;
    std::cout << "  User transports count: " << pqos.transport().user_transports.size() << std::endl;
    std::cout << "  Use builtin transports: " << (pqos.transport().use_builtin_transports ? "true" : "false") << std::endl;
    for (const auto& transport : pqos.transport().user_transports) {
        // 通过 dynamic_cast 判断传输类型
        if (std::dynamic_pointer_cast<eprosima::fastdds::rtps::UDPv4TransportDescriptor>(transport)) {
            std::cout << "  Transport type: UDPv4" << std::endl;
        } else if (std::dynamic_pointer_cast<eprosima::fastdds::rtps::SharedMemTransportDescriptor>(transport)) {
            std::cout << "  Transport type: SHM" << std::endl;
        } else {
            std::cout << "  Transport type: Unknown" << std::endl;
        }
    }

    // Register the type for domain participant

...
public:
    ...
    static void print_help(const uint8_t return_code) {
        ...
        std::cout << "                                        <app_path>/static_edp_discovery.xml)" << std::endl;
     ...
     */
    static static_edp_discovery_config parse_cli_options(int argc, char* argv[]) {
        ...
        for (int i = 2; i < argc; ++i) {
            if (std::string arg = argv[i]; arg == "-h" || arg == "--help") {
                ...
                if (++i < argc) {
                    try {
                        if (int input = std::stoi(argv[i]); input < 0 || input > 232) {
                            throw std::out_of_range("domain argument " + std::string(argv[i]) + " out of range [0, 232].");
                        } else {
                            config.domain = static_cast<uint16_t>(input);
                        }
                    } catch (const std::invalid_argument& e) {
                        EPROSIMA_LOG_ERROR(CLI_PARSER,
                                           "invalid domain argument " + std::string(argv[i]) + ": " + std::string(e.what()));
                        print_help(EXIT_FAILURE);
                ...
                if (++i < argc) {
                    try {
                        if (int input = std::stoi(argv[i]);
                            input < std::numeric_limits<uint16_t>::min() || input > std::numeric_limits<uint16_t>::max()) {
                            throw std::out_of_range("sample argument " + std::string(argv[i]) + " out of range [0, 65535].");
                        } else {
                            config.samples = static_cast<uint16_t>(input);
                        }
                    } catch (const std::invalid_argument& e) {
                        EPROSIMA_LOG_ERROR(CLI_PARSER,
                                           "invalid sample argument " + std::string(argv[i]) + ": " + std::string(e.what()));
                        print_help(EXIT_FAILURE);
                    } catch (const std::out_of_range& e) {
                        EPROSIMA_LOG_ERROR(CLI_PARSER, std::string(e.what()));
                        print_help(EXIT_FAILURE);
        ...
        if (config.xml_path.empty()) {
            config.xml_path = "file://" + get_path("static_edp_discovery.xml");
        } else {
            config.xml_path = "file://" + config.xml_path;
        }
        std::cout << "xml_path: " << config.xml_path << std::endl;
        return config;
    }

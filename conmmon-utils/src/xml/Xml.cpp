...
namespace common_utils{
    ...
    void Xml::saveXmlDocument(tinyxml2::XMLDocument& doc, const std::string& path) {
        ...
        //在文件名加上_modified
        const std::string modifiedPath = dir + "/"
    ...
    // 根据名称查找 AR-PACKAGE
    tinyxml2::XMLElement*
    Xml::findArPackage(tinyxml2::XMLElement* arPackages, const std::string& shortName) {
        if (!arPackages) {
    ...
    std::vector<tinyxml2::XMLElement*> Xml::findAllArPackages(tinyxml2::XMLElement*& ar_packages) {
        ...
        for (auto ar_pkg = ar_packages->FirstChildElement();
             ar_pkg; ar_pkg = ar_pkg->NextSiblingElement()) {
    ...
    tinyxml2::XMLElement* Xml::findElement(tinyxml2::XMLElement* arPkg, const std::string& name,
    ...
    void Xml::removeAllDescriptions(tinyxml2::XMLElement* arPkgs, const std::string& tag) {
        ...
        while (const auto child = arPkgs->FirstChildElement(tag.c_str())) {
            ...
            LOG(INFO) << "已删除元素 " << arPkgs->Name()
                << " 的 " << tag << " (第" << arPkgs->GetLineNum() << "行)";
        }

        // 2. 深度优先遍历所有子元素
        for (auto child = arPkgs->FirstChildElement();
             child;

...
std::string Core::getExeDirectory() {
    char exePath[4096];
#ifdef _WIN32
    if (const DWORD ret = GetModuleFileNameA(nullptr, exePath, sizeof(exePath)); ret == 0 || ret >= sizeof(exePath)) return "";
#else
...
std::string Core::getCurrentTimestamp(const std::string& format, const bool ms) {
    ...
    if (!ms) {
        ...
        const auto ms1 = std::chrono::duration_cast<std::chrono::milliseconds>(now_ms.time_since_epoch()).count() %
            1000; // 模运算，获取毫秒数（1s=1000ms）

        // 在末尾添加毫秒，std::setfill('0') 用于设置填充字符，通常与 std::setw() 配合使用，当输出内容宽度不足时，用 '0'
        // 填充剩余空间。

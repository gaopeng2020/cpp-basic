...
public:
    posix_chat_client(asio::io_context& io_context, const tcp::resolver::results_type& endpoints) :
        socket_(io_context),
...
private:
    ...
    void do_write_output() {
        ...
        std::array<asio::const_buffer, 2> buffers = {
            {asio::buffer(read_msg_.body(), read_msg_.body_length()), asio::buffer(eol)}};
        asio::async_write(output_, buffers, [this](std::error_code ec, std::size_t /*length*/) {
            if (!ec) {
                do_read_header();
            } else {
                close();
    ...
    void do_write_message() {
        asio::async_write(socket_, asio::buffer(write_msg_.data(), write_msg_.length()),
                          [this](std::error_code ec, std::size_t /*length*/) {
                              if (!ec) {
                                  do_read_input();
                              } else {
